...
class MainPorgamm:
    ...
    def setReferencePoint(self, Point, keepTracking = True):      
        '''Methode zum Setzen des Referenzpunktes (für Drehimpuls etc.). Soll der RefP
        immer durch die WiiMote getrackt werden, wird keepTracking auf True gesetzt und in self.ReferencePoint die
        ID des getrackten Punkt gespeichtert. Soll nur einmalig durch die WiiMotegesetzt werden wird in self.ReferencePoint
        ein Vektor mit den Koordinaten des Punktes gespeichert. Ist kein RefP gewählt sind beide Vablen auf none gesetzt.''' 
        self.TrackReferencePoint = keepTracking
        if keepTracking:
            self.ReferencePoint = Point.ID()
        else:
            self.ReferencePoint = QVector2D(Point.toPointF())
